...
class JoinedCoursesWidget extends StatefulWidget {
  final Map<String, dynamic>? studentData;
  final VoidCallback? onCourseLeft;

  const JoinedCoursesWidget({
    Key? key,
    this.studentData,
    this.onCourseLeft,
  }) : super(key: key);
  ...
  @override
  _JoinedCoursesWidgetState createState() => _JoinedCoursesWidgetState();
}

class _JoinedCoursesWidgetState extends State<JoinedCoursesWidget> {
  ...
  Future<void> _loadJoinedCourses() async {
    ...
    try {
            ...
            if (courseDoc.exists) {
              ...
              if (courseData != null) {
                joinedCourses.add({
                  ...
                  'session': courseData['session'] ?? 'Day',
      ...
      print('Error loading joined courses: $e');
      setState(() => _isLoading = false);
    }
  }
  ...
  Future<void> _leaveCourse(String courseId) async {
    ...
    try {
      ...
      await studentCourseRef.delete();

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Successfully left the course!')),
      );
      ...
      // Notify parent
      if (widget.onCourseLeft != null) {
        widget.onCourseLeft!();
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to leave course: $e')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_joinedCourses.isEmpty) {
      return const Center(
        child: Text('You haven\'t joined any courses yet.'),
      );
    }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: isExpired ? null : () => _leaveCourse(course['id']),
